...
**Tasks:**
1. Initialize repo with `electron-vite` (React + TypeScript template). Three entry points: `main`, `preload`, `renderer`.
2. Configure `tsconfig` with `strict: true` across all three processes; set up path aliases (`@main`, `@renderer`, `@shared`).
3. Install and configure Tailwind CSS; define a custom theme token for the soft light-blue accent (`primary`).
4. Initialize Shadcn UI; install base primitives needed across the app (Button, Switch, Select, Slider, Card, Toast).
5. Configure ESLint (typescript-eslint, react-hooks) and Prettier; add lint + format scripts.
6. Install Framer Motion; render a one-off animated component to verify the renderer pipeline works.
7. Create `@shared/schemas` module — Zod schemas for the full settings object and every IPC channel payload.
8. Configure `electron-store` with the Zod-derived TypeScript type as its schema; expose a typed `settingsStore` singleton from main.
9. Implement `preload.ts` using `contextBridge.exposeInMainWorld('horizon', api)`. Define a typed API surface (no Node leakage). Set `contextIsolation: true`, `sandbox: true`, `nodeIntegration: false`.
10. Build a generic IPC handler registry in main: every channel goes through Zod validation; invalid payloads reject with a typed error.
11. Implement `settings:get`, `settings:set`, `settings:subscribe` channels end-to-end.
12. Build a temporary dev panel in the settings window that round-trips a setting value (write → store → IPC subscribe → renderer re-render). This is the M1 acceptance check.
13. Configure the main settings window: standard frame, `width: 720`, `height: 720`, closable hides instead of quits.
